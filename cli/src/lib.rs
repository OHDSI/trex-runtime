...
use base::server::{Builder, ServerFlags, WorkerEntrypoints, ServerHealth, Tls};
...
impl std::fmt::Debug for ServerConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ServerConfig")
            ...
            .field("graceful_exit_deadline_sec", &self.graceful_exit_deadline_sec)
            .field("graceful_exit_keepalive_deadline_ms", &self.graceful_exit_keepalive_deadline_ms)
            .field("event_worker_exit_deadline_sec", &self.event_worker_exit_deadline_sec)
            ...
            .field("beforeunload_wall_clock_pct", &self.beforeunload_wall_clock_pct)
...
pub struct ServerHandle {
    pub id: String,
    pub addr: SocketAddr,
    health_rx: mpsc::UnboundedReceiver<base::server::ServerEvent>,
}

impl ServerHandle {
    pub fn new(id: String, addr: SocketAddr, health_rx: mpsc::UnboundedReceiver<base::server::ServerEvent>) -> Self {
        Self {
            id,
            addr,
            health_rx,
...
pub async fn start_server(config: ServerConfig) -> Result<ServerHandle, Error> {
    ...
    *builder.entrypoints_mut() = config.to_worker_entrypoints();
    ...
    let server_id = format!("trex_{}_{}", server_addr.port(), chrono::Utc::now().timestamp());
    
    // Register the server with the global manager
    get_global_server_manager().register_server(server_id.clone(), config.clone())?;
    ...
    tokio::spawn(async move {
        ...
        let _ = get_global_server_manager().unregister_server(&server_id_for_cleanup);
    ...
    Ok(ServerHandle::new(server_id, server_addr, health_rx))
}
