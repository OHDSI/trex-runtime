...
use crate::conversions::table::{
  ColumnSchema, TableId, TableName, TableSchema,
};
...
impl ReplicationClient {
  ...
  pub async fn get_column_schemas(
      ...
      .await?
    {
      if let SimpleQueryMessage::Row(row) = message {
        ...
        let nullable = row.try_get("attnotnull")?.ok_or(
        ...
        let primary = row.try_get("primary")?.ok_or(
          ReplicationClientError::MissingColumn(
  ...
  pub async fn get_table_id(
    ...
    for message in self.postgres_client.simple_query(&table_info_query).await? {
      if let SimpleQueryMessage::Row(row) = message {
        let replica_identity = row.try_get("relreplident")?.ok_or(
          ReplicationClientError::MissingColumn(
  ...
  async fn get_slot(
    &self,
    slot_name: &str,
  ) -> Result<Option<SlotInfo>, ReplicationClientError> {
    let query = format!(
