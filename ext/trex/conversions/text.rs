...
use super::{
  bool::ParseBoolError, hex::ByteaHexParseError, numeric::PgNumeric, ArrayCell,
  Cell,
};
...
impl TextFormatConverter {
  pub fn default_value(typ: &Type) -> Cell {
    match *typ {
      ...
      Type::TIMESTAMP_ARRAY => {
        Cell::Array(ArrayCell::TimeStamp(Vec::default()))
      }
      Type::TIMESTAMPTZ => {
        let val =
      ...
      Type::TIMESTAMPTZ_ARRAY => {
        Cell::Array(ArrayCell::TimeStampTz(Vec::default()))
      }
      ...
      Type::JSON_ARRAY | Type::JSONB_ARRAY => {
        Cell::Array(ArrayCell::Json(Vec::default()))
      }
  ...
  pub fn try_from_str(typ: &Type, str: &str) -> Result<Cell, FromTextError> {
    match *typ {
      ...
      Type::INT2_ARRAY => TextFormatConverter::parse_array(
        str,
        |str| Ok(Some(str.parse()?)),
        ArrayCell::I16,
      ),
      Type::INT4 => Ok(Cell::I32(str.parse()?)),
      Type::INT4_ARRAY => TextFormatConverter::parse_array(
        str,
        |str| Ok(Some(str.parse()?)),
        ArrayCell::I32,
      ),
      Type::INT8 => Ok(Cell::I64(str.parse()?)),
      Type::INT8_ARRAY => TextFormatConverter::parse_array(
        str,
        |str| Ok(Some(str.parse()?)),
        ArrayCell::I64,
      ),
      Type::FLOAT4 => Ok(Cell::F32(str.parse()?)),
      Type::FLOAT4_ARRAY => TextFormatConverter::parse_array(
        str,
        |str| Ok(Some(str.parse()?)),
        ArrayCell::F32,
      ),
      Type::FLOAT8 => Ok(Cell::F64(str.parse()?)),
      Type::FLOAT8_ARRAY => TextFormatConverter::parse_array(
        str,
        |str| Ok(Some(str.parse()?)),
        ArrayCell::F64,
      ),
      ...
      Type::OID_ARRAY => TextFormatConverter::parse_array(
        str,
        |str| Ok(Some(str.parse()?)),
        ArrayCell::U32,
      ),
