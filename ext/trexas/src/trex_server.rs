...
pub use trex_cli::{ServerConfig, ServerManager, get_version, get_global_server_manager};

static LOG_INIT: AtomicBool = AtomicBool::new(false);

static SERVER_THREADS: LazyLock<Arc<Mutex<HashMap<String, thread::JoinHandle<()>>>>> = LazyLock::new(|| {
...
impl TrexServerManagerWrapper {
    ...
    pub fn start_server_with_defaults(&self, addr: Option<SocketAddr>, main_service_path: Option<String>) -> Result<String> {
    ...
    fn start_server_persistent(&self, config: ServerConfig) -> Result<String> {
        ...
        let server_id = format!("trex_{}_{}", config.addr.port(), chrono::Utc::now().timestamp());
        ...
        let thread_handle = thread::spawn(move || {
            ...
            let result = local.block_on(&runtime, async {
                let mut builder = Builder::new(config_clone.addr, &config_clone.main_service_path);
                
                if let (Some(cert_path), Some(key_path)) = (&config_clone.tls_cert_path, &config_clone.tls_key_path) {
                ...
                *builder.entrypoints_mut() = entrypoints;
                ...
                if let Err(e) = get_global_server_manager().register_server(server_id_clone.clone(), config_clone.clone()) {
                    return Err(e);
                }
    ...
    fn create_tls_config_static(cert_path: &str, key_path: &str) -> Result<base::server::Tls> {
        use std::fs;
...
impl TrexServerManagerWrapper {
    ...
    pub fn list_servers(&self) -> Vec<(String, ServerHandle)> {
        match self.manager.list_servers() {
            Ok(servers) => {
                servers.into_iter().map(|(id, config, _status)| {
...
pub static TREX_MANAGER: LazyLock<TrexServerManagerWrapper> = LazyLock::new(|| {
    init_logging();
...
fn default_main_service_path() -> String { "main.ts".to_string() }
