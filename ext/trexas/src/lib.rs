...
use trex_server::{TREX_MANAGER, TrexServerConfig};
...
impl VScalar for StartTrexServerScalar {
    ...
    unsafe fn invoke(
        ...
        let main_service_slice = main_service_vector.as_slice_with_len::<libduckdb_sys::duckdb_string_t>(input.len());
        let event_worker_slice = event_worker_vector.as_slice_with_len::<libduckdb_sys::duckdb_string_t>(input.len());
        ...
        let main_service_path = duckdb::types::DuckString::new(&mut { main_service_slice[0] }).as_str().to_string();
        let event_worker_path = duckdb::types::DuckString::new(&mut { event_worker_slice[0] }).as_str().to_string();
...
impl VScalar for StartTrexServerWithConfigScalar {
    ...
    unsafe fn invoke(
        ...
        let config_slice = config_vector.as_slice_with_len::<libduckdb_sys::duckdb_string_t>(input.len());
        
        if input.len() == 0 {
            return Err("No input provided".into());
        }
        
        let config_json_str = duckdb::types::DuckString::new(&mut { config_slice[0] }).as_str().to_string();
        
        let response = match serde_json::from_str::<TrexServerConfig>(&config_json_str) {
            Ok(config_struct) => {
                match config_struct.to_server_config() {
                    Ok(server_config) => {
...
impl VScalar for StopTrexServerScalar {
    ...
    unsafe fn invoke(
        ...
        let server_id_slice = server_id_vector.as_slice_with_len::<libduckdb_sys::duckdb_string_t>(input.len());
        
        if input.len() == 0 {
            return Err("No input provided".into());
        }
        
        let server_id = duckdb::types::DuckString::new(&mut { server_id_slice[0] }).as_str().to_string();
...
impl VTab for TrexServersTable {
    ...
    fn bind(bind: &BindInfo) -> Result<Self::BindData, Box<dyn std::error::Error>> {
        bind.add_result_column("server_id", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        bind.add_result_column("ip", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        bind.add_result_column("port", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        bind.add_result_column("main_service_path", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        bind.add_result_column("event_worker_path", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        bind.add_result_column("started_at", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        bind.add_result_column("policy", LogicalTypeHandle::from(LogicalTypeId::Varchar));
    ...
    fn func(func: &TableFunctionInfo<Self>, output: &mut DataChunkHandle) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (i, (server_id, handle)) in servers.iter().enumerate() {
            ...
            let main_service_cstring = CString::new(handle.config.main_service_path.as_str())?;
            let event_worker_cstring = CString::new(
                handle.config.event_worker_path.as_deref().unwrap_or("none")
            )?;
            let started_at_cstring = CString::new(handle.started_at.format("%Y-%m-%d %H:%M:%S UTC").to_string())?;
            let policy_cstring = CString::new(
                handle.config.user_worker_policy.as_ref()
                    .map(|_| "user-defined")
                    .unwrap_or("default")
            )?;
