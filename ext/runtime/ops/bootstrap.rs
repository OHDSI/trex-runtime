  ...
  P: FsPermissions + 'static,
{
  ...
  let cwd = std::env::current_dir().map_err(|e| JsErrorBox::type_error(e.to_string()))?;
  ...
  if main_url.scheme() == "file" {
    let main_path = std::env::current_dir()
      .map_err(|e| JsErrorBox::type_error(format!("Failed to get current working directory: {}", e)))?
      .join(main_url.to_string());
    state.borrow_mut::<P>().check_read_all("Deno.mainModule")
