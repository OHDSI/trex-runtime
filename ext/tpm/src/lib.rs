...
impl VTab for TpmInfoVTab {
    ...
    fn bind(bind: &BindInfo) -> Result<Self::BindData, Box<dyn std::error::Error>> {
        // Output column: JSON string with package info
        bind.add_result_column("package_info", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        let package_name = bind.get_parameter(0).to_string();
        // Check for TPM_REGISTRY_URL environment variable
        let registry_url = std::env::var("TPM_REGISTRY_URL").ok();
        Ok(TpmInfoBindData { package_name, registry_url })
    }
    ...
    fn func(func: &TableFunctionInfo<Self>, output: &mut DataChunkHandle) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let registry = npm::NpmRegistry::with_registry_url(bind_data.registry_url.clone())?;
...
impl VTab for TpmResolveVTab {
    ...
    fn bind(bind: &BindInfo) -> Result<Self::BindData, Box<dyn std::error::Error>> {
        bind.add_result_column("resolve_info", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        let package_spec = bind.get_parameter(0).to_string();
        // Check for TPM_REGISTRY_URL environment variable
        let registry_url = std::env::var("TPM_REGISTRY_URL").ok();
        Ok(TpmResolveBindData { package_spec, registry_url })
    }
...
impl VTab for TpmInstallVTab {
    ...
    fn bind(bind: &BindInfo) -> Result<Self::BindData, Box<dyn std::error::Error>> {
        bind.add_result_column("install_result", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        let package_spec = bind.get_parameter(0).to_string();
        let install_dir = bind.get_parameter(1).to_string();
        // Check for TPM_REGISTRY_URL environment variable
        let registry_url = std::env::var("TPM_REGISTRY_URL").ok();
        Ok(TpmInstallBindData { package_spec, install_dir, registry_url })
    }
    ...
    fn func(func: &TableFunctionInfo<Self>, output: &mut DataChunkHandle) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let registry = npm::NpmRegistry::with_registry_url(bind_data.registry_url.clone())?;
        let result = registry.install_package(&bind_data.package_spec, &bind_data.install_dir);
...
impl VTab for TpmInstallDepsVTab {
    ...
    fn bind(bind: &BindInfo) -> Result<Self::BindData, Box<dyn std::error::Error>> {
        bind.add_result_column("install_results", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        let package_spec = bind.get_parameter(0).to_string();
        let install_dir = bind.get_parameter(1).to_string();
        // Check for TPM_REGISTRY_URL environment variable
        let registry_url = std::env::var("TPM_REGISTRY_URL").ok();
        Ok(TpmInstallDepsBindData { package_spec, install_dir, registry_url })
    }

    fn init(init: &InitInfo) -> Result<Self::InitData, Box<dyn std::error::Error>> {
        let bind_data = init.get_bind_data::<Self::BindData>();

        let registry = unsafe { npm::NpmRegistry::with_registry_url((*bind_data).registry_url.clone())? };
        let results = unsafe {
            registry.install_package_with_deps(&(*bind_data).package_spec, &(*bind_data).install_dir)?
        };
    ...
    fn func(func: &TableFunctionInfo<Self>, output: &mut DataChunkHandle) -> Result<(), Box<dyn std::error::Error>> {
        let init_data = func.get_init_data();
        let current_index = init_data.index.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
...
impl VTab for TpmTreeVTab {
    ...
    fn bind(bind: &BindInfo) -> Result<Self::BindData, Box<dyn std::error::Error>> {
        bind.add_result_column("tree_info", LogicalTypeHandle::from(LogicalTypeId::Varchar));
        let package_spec = bind.get_parameter(0).to_string();
        let registry_url = std::env::var("TPM_REGISTRY_URL").ok();
        Ok(TpmTreeBindData { package_spec, registry_url })
    }
    ...
    fn init(init: &InitInfo) -> Result<Self::InitData, Box<dyn std::error::Error>> {
        ...
        let registry = unsafe { npm::NpmRegistry::with_registry_url((*bind_data).registry_url.clone())? };
        let results = unsafe {
            registry.get_dependency_tree(&(*bind_data).package_spec)?
        };
...
impl VTab for TpmListVTab {
    ...
    fn bind(bind: &BindInfo) -> Result<Self::BindData, Box<dyn std::error::Error>> {
        bind.add_result_column("list_info", LogicalTypeHandle::from(LogicalTypeId::Varchar));
...
pub unsafe fn extension_entrypoint(con: Connection) -> Result<(), Box<dyn Error>> {
    ...
    con.register_table_function::<TpmInfoVTab>("tpm_info")
        .expect("Failed to register tpm_info table function");

    con.register_table_function::<TpmResolveVTab>("tpm_resolve")
        .expect("Failed to register tpm_resolve table function");

    con.register_table_function::<TpmInstallVTab>("tpm_install")
        .expect("Failed to register tpm_install table function");

    con.register_table_function::<TpmInstallDepsVTab>("tpm_install_with_deps")
        .expect("Failed to register tpm_install_with_deps table function");

    con.register_table_function::<TpmTreeVTab>("tpm_tree")
        .expect("Failed to register tpm_tree table function");

    con.register_table_function::<TpmListVTab>("tpm_list")
