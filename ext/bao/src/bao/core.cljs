...
(defn -main
  ...
  (let [cwd (js/process.cwd)
        ;; nbb puts -m bao.core in the args, so we need to handle -- separator
        argv-array (.-argv js/process)
        dash-dash-idx (.indexOf argv-array "--")
        ;; If there's a --, take everything after it, otherwise take from index 2
        args-to-parse (if (>= dash-dash-idx 0)
                        (.slice argv-array (inc dash-dash-idx))
                        (.slice argv-array 2))
...
")
    ...
    (let [trexas-host (:trexas-host args-map)
      ...
      (-> (p/do!
            ...
            (println "\n🚀 Starting servers...")
            ;; Start pgwire server
            (p/let [pgwire-result (query conn (str "SELECT start_pgwire_server('" pgwire-host "', " pgwire-port ", 'password', '') as result"))]
